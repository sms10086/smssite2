...
public class MTImmedisteThread extends WorkThread{	private static final Log LOG = LogFactory.getLog(MTImmedisteThread.class);
	private  boolean isbreak=false;
	private boolean isbig=false;
	 
	public MTImmedisteThread(boolean isbig ){
		this.isbig=isbig;
	...
	public void work() throws Exception {
			...
			LOG.info(">>>>>>>>>>>>>>>>>>>MTImmedisteThread begin,isbig="+isbig);
		 ...
		 while(isWork()){
			 ...
			 LOG.info(">>>>>>>>>>>>>>>>>>>MTImmedisteThread end,isbig="+isbig);
	...
	public void MTDate(){
		...
		try{
			...
			if(isbig)
				prep=conn.prepareStatement("select * from task where flag=1 and scheduleTime is null   and phoneNum>=20000 order by phonenum asc",ResultSet.TYPE_FORWARD_ONLY,
		...
		String[] contents=null;
			...
			while (phoneRs.next()) {
				...
				if (total % 50 == 0) {
					...
					sleep(10);
					LOG.info(">>>>submit 50 tiao.>>>>"+System.currentTimeMillis());
				}
				if(!isWork()) {
					isbreak=true;
					break;
